<?php
class PaginateFormComponent extends Object {
	...
	function engage($defaultFormData = array(),$removeNullValues = true){
		...
		if($removeNullValues){
			...
			if(!empty($this->controller->request->data)){
				...
				$this->controller->request->data = $this->filterNullValues($this->controller->request->data);
			}
		}

		///merge defaultFormData with post data (if any), else set to defaults:
		$this->controller->request->data = ($this->controller->request->data)? array_merge($defaultFormData, $this->controller->request->data) : $defaultFormData;

		//convert form data to passedArgs for paginate
		$this->nonPaginatorArgs = $this->convertDataToPassedArgs($this->controller->request->data);
		if(!empty($this->controller->passedArgs)){
			//if passedArgs exist, indicates a get request from the paginator.
			//convert back to $this->controller->request->data for form auto population, removing paginator vals
			...
			$this->controller->request->data = $this->convertPassedArgsToData($this->nonPaginatorArgs);
